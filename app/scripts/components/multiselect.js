			...
			link    : function(originalScope, element, attrs, modelCtrl) {
				...
				function getHeaderText() {
					if(isEmpty(modelCtrl.$modelValue)) {
						scope.header = attrs.msHeader || "Selecione";
						return scope.header;
					}
					...
					if(isMultiple) {
						if(attrs.msSelected) {
							...
							if(modelCtrl.$modelValue.length < 2){
...
angular.module("multiselect.tpl.html", []).run(["$templateCache", function($templateCache) {
	$templateCache.put("multiselect.tpl.html",
			"<div class=\"btn-group multiple-select-box\">\n" +
			...
			"    <div ng-style=\"maxWidth\" style=\"padding-right: 13px; overflow: hidden; text-overflow: ellipsis;\">{{header}}</div><span class=\"caret\" style=\"position:absolute;right:10px;top:24px;\"></span>\n" +
			"  </button>\n" +
